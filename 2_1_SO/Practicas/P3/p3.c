...
void Cmd_open(char *tr[], struct ProcessList PList[]) {
    int i, df, mode = 0;
    ...
    // Abre el proceso con el nombre del fichero si está definido
    df = open(tr[1], mode, 0777);

    if (df == -1) {
        error("Imposible abrir fichero\n");
    } else {
        add_open_file(mode, tr[1], PList);
    }
}
...
void CloseAllOpen(struct ProcessList PList[]) {
    int df;

    for(df=3; df < MAXOPEN; df++){
        if(strcmp(PList[df].name, "") != 0) {
            close(df);
            strcpy(PList[df].name, "");
            PList[df].mode = 0;
...
struct CommandsStruct {
    ...
    char info[MAXCN][300]; // Un array de cadenas de caracteres con un tamaño máximo de 10 elementos y cada cadena puede contener hasta 50 caracteres
};
...
void StartStruct(struct CommandsStruct *CoSt){
    ...
    //P3
    strcpy(CoSt->commands[30], "uid");
    strcpy(CoSt->info[30], "uid [-get|-set] [-l] [id]\tAccede a las credenciales del proceso que ejecuta el shell\n\t-get: muestra las credenciales\n\t-set id: establece la credencial al valor numerico id\n\t-set -l id: establece la credencial a login id");
    strcpy(CoSt->commands[31], "showvar");
    strcpy(CoSt->info[31], "showvar var\tMuestra el valor y las direcciones de la variable de entorno var\n");
    strcpy(CoSt->commands[32], "changevar");
    strcpy(CoSt->info[32], "changevar [-a|-e|-p] var valor\tCambia el valor de una variable de entorno\n\t-a: accede por el tercer arg de main\n\t-e: accede mediante environ\n\t-p: accede mediante putenv");
    strcpy(CoSt->commands[33], "subsvar");
    strcpy(CoSt->info[33], "subsvar [-a|-e] var1 var2 valor\tSustituye la variable de entorno var1\n\tcon var2=valor\n\t-a: accede por el tercer arg de main\n\t-e: accede mediante environ");
    strcpy(CoSt->commands[34], "showenv");
    strcpy(CoSt->info[34], "showenv [-environ|-addr] \t Muestra el entorno del proceso\n\t-environ: accede usando environ (en lugar del tercer arg de main)\n\t-addr: muestra el valor y donde se almacenan environ y el 3er arg main ");
    strcpy(CoSt->commands[35], "fork");
    strcpy(CoSt->info[35], "fork \tEl shell hace fork y queda en espera a que su hijo termine");
    strcpy(CoSt->commands[36], "exec");
    strcpy(CoSt->info[36], "exec VAR1 VAR2 ..prog args....[@pri]\tEjecuta, sin crear proceso,prog con argumentos\n\t en un entorno que contiene solo las variables VAR1, VAR2...");
    strcpy(CoSt->commands[37], "jobs");
    strcpy(CoSt->info[37], "jobs \tLista los procesos en segundo plano");
    strcpy(CoSt->commands[38], "deljobs");
    strcpy(CoSt->info[38], "deljobs [-term][-sig]\tElimina los procesos de la lista procesos en sp\n\t-term: los terminados\n\t-sig: los terminados por senal");
    strcpy(CoSt->commands[39], "job");
    strcpy(CoSt->info[39], "job [-fg] pid\tMuestra informacion del proceso pid.\n\t\t-fg: lo pasa a primer plano");
...
void CmdMmap(char *arg[], struct MmapStruct MapStr[]){
...
void Mmap(char *tr[], struct MmapStruct MapStr[]){
    CmdMmap(tr,MapStr);
}
...
void memfill(char * tr[], struct MemoryStruct MemStr[]){
    if(tr[1] != NULL && tr[2] != NULL && tr[3] != NULL){ //memfill 0x0 10 'a'
        ...
        for (i = 0; i < MAXOPEN; i++) {
            ...
            if(i==MAXOPEN-1) {
                printf("Limite de mallocs alcanzado\n");
            }
        }
...
void uid_operations(char *tr[]) {
    if (tr[1] == NULL || strcmp(tr[1], "-get") == 0) {
        ...
        if(tr[2]==NULL){
            ...
            int new_uid = atoi(tr[2]);
            if (seteuid(new_uid) == -1) {
                error("Error al establecer la credencial efectiva");
            }else printf("Credencial efectiva establecida a: %d, (%s)\n", geteuid(), getlogin());
        } else if ( tr[4] == NULL && strcmp(tr[2], "-l") == 0) {
            int new_uid = atoi(tr[3]);
            struct passwd *pwd = getpwnam(tr[3]);
            if (pwd == NULL) {
                fprintf(stderr, "Error: Usuario '%s' no encontrado.\n", tr[3]);
            } else new_uid = pwd->pw_uid;
        ...
        printf("Uso: %s [-get | -set id | -set -l id]\n", tr[0]);
    }
}
...
void sshowvar(char *tr[], char *entorno[]){
    int i;
    if(tr[1]==NULL){
        MuestraEntorno(entorno, "main arg3");
    }else{
        i = BuscarVariable(tr[1], entorno);
        if(i==-1) printf("Esa variable no existe\n");
        else {
...
void changevarr(char *tr[], char *environ[]) {
    if (tr[1] == NULL) {
        ...
        if (strcmp(opcion, "-a") == 0) {
            if (var != NULL && valor != NULL) {
                int pos = CambiarVariable(var, valor, environ);

                if (pos != -1) {
                    printf("Valor de %s cambiado a %s\n", var, valor);
                } else {
                    if (errno == ENOENT) {
                        printf("Variable '%s' no encontrada.\n", var);
                    } else {
                        error("Error al cambiar el valor de la variable de entorno");
                    }
                }
            }
            ...
            if (var != NULL && valor != NULL) {
                ...
                printf("Variable %s modificada a %s\n", var, valor);
            }
        }else {
            printf("Uso incorrecto: changevar -p var valor\n");
        }
    }
}

void subsvarr(char *tr[], char *environ[]) {
    if (tr[1] == NULL || tr[2] == NULL || tr[3] == NULL || tr[4] == NULL) {
        ...
        if (strcmp(opcion, "-a") == 0) {
            // Modo -a: acceder por el tercer argumento de main
            int posVar1 = BuscarVariable(var1, environ);

            if (posVar1 != -1) {
                // var1 tiene el valor esperado, crear var2 con valor
                char *aux;
                ...
                environ[posVar1] = aux;
                printf("Sustituido %s con %s=%s\n", var1, var2, valor);
            } else {
                printf("No se cumple la condición %s=%s\n", var1, valor);
            }
        } else if (strcmp(opcion, "-e") == 0) {
            // Modo -e: acceder mediante environ
            int posVar1 = BuscarVariable(var1, environ);
            int posVar2 = BuscarVariable(var2, environ);

            if (posVar2 != -1) {
                printf("Error: La variable %s ya existe en el entorno.\n", var2);
            }
...
void showenv(char *tr[], char *entorno[]) {
    if (tr[1] == NULL) {
        MuestraEntorno(entorno, "main");
    }else {

        char *opcion = tr[1];

        if (strcmp(opcion, "-environ") == 0) {
            MuestraEntorno(entorno, "environ");
        } else if (strcmp(opcion, "-addr") == 0) {
            // Modo -addr: mostrar direcciones de environ y el tercer arg de main
...
void listjobs(struct ProcForkList ForkList[]){
    int status;
    bool conocido = false;
    for (int i = 0; i < MAXOPEN; i++) {
        if(ForkList[i].pid >=0) {
            ForkList[i].prioridad = getpriority(PRIO_PROCESS, ForkList[i].pid);

            if (waitpid(ForkList[i].pid, &status, WNOHANG | WUNTRACED | WCONTINUED) == ForkList[i].pid) {
                conocido = false;
                if (WIFEXITED(status)) {
                } else if (WIFSTOPPED(status)) {
                    strcpy(ForkList[i].nombresenal, "PARADA");
                    conocido = true;
                } else if (WIFSIGNALED(status)) {
                } else if (WIFCONTINUED(status)) {
                    strcpy(ForkList[i].nombresenal, "ACTIVA");
                    ForkList[i].senal = 0;
                    conocido = true;
                }

                if(status == 0) {
                    strcpy(ForkList[i].nombresenal, "TERMINADO");
                    ForkList[i].senal = 0;
                } else {
                    ForkList[i].senal = WTERMSIG(status);
                    if (!conocido){
                        strcpy(ForkList[i].nombresenal,NombreSenal(ForkList[i].senal));
                    }
                }
            }
            printf("  %d\t\t%s p=%d %s %s (%03d) %s\n", ForkList[i].pid, ForkList[i].usuario, ForkList[i].prioridad, ForkList[i].hora, ForkList[i].nombresenal, ForkList[i].senal, ForkList[i].linea);
        }
    }
}

void deljobs(char * tr[], struct ProcForkList ForkList[]){
    bool term = false, sig = false;
    if(tr[1] == NULL){
        listjobs(ForkList);
        return;
    }
    if(strcmp(tr[1], "-sig")==0){
        sig = true;
    }
    ...
    for (int i = 0; i < MAXOPEN; i++) {
        if(ForkList[i].pid >=0){
            if(term){
                if(ForkList[i].senal == 0 && strcmp(ForkList[i].nombresenal, "ACTIVO")!=0)
                    ForkList[i].pid = -1; //Se indica que termino
            } else if (sig){
                if(ForkList[i].senal != 0)
                    ForkList[i].pid = -1; //Se indica que termino
            }
        }
    }
...
void exec(char * tr[]){
    if(tr[1] != NULL)
        execvp(tr[1], &tr[1]);
    error("Imposible ejecutar");
}

void job(char * tr[], struct ProcForkList ForkList[]) {
    int i = 1, pidbuscar, status;
    bool fg = false;
    if(tr[1]==NULL){
        listjobs(ForkList);
        return;
    ...
    for (i = 0; i < MAXOPEN; i++) {
        if (ForkList[i].pid == pidbuscar) break;
    }
    if (i == MAXOPEN) listjobs(ForkList);
    ...
    if(fg){
        ...
        if (waitpid(ForkList[i].pid, &status, WNOHANG) == ForkList[i].pid) {
            if (WIFEXITED(status)) {
            } else if (WIFSIGNALED(status)) {
            } else if (WIFSTOPPED(status)) {
            } else if (WIFCONTINUED(status)) {
...
void VaciarListaProcesos(struct ProcForkList ForkList[]) {
    for (int i = 0; i < MAXOPEN; i++) {
        if(ForkList[i].pid >=0) ForkList[i].pid = -1;
    }
}

void Cmd_fork (struct ProcForkList ForkList[]) {
    pid_t pid;

    if ((pid=fork())==0){
		VaciarListaProcesos(ForkList);
...
void lastcheck(char * tr[], struct ProcForkList ForkStr[]){
    ...
    pid_t pid = fork();
    for (i = 0; tr[i] != NULL; i++) {
        if (strcmp(tr[i], "&") == 0) { // & para segundo plano y @ para la prioridad
            segundoplano = true;
            for (j = i; tr[j+1] != NULL; j++) { // Muevo todos los argumentos siguientes una posicion atras
                memmove(tr[j], tr[j+1], strlen(tr[j+1]) + 1);
            }
            tr[j] = NULL; //Establezco como nulo el ultimo porque termine de desplazarlos
            i--; //Tiene que repetir la iteracion, ya que se puede repetir una de estas condiciones de nuevo
        }
    }

    if (pid == -1) {
        error("No se pudo crear otro proceso");
    } else if (pid == 0) { //Esto lo hace el hijo
        int resultado = execvp(tr[0], tr);
        if (resultado == -1) {
            error("No ejecutado");
            exit(EXIT_FAILURE);
            return;
        ...
        if (!segundoplano) {
            waitpid(pid, NULL, 0);
        } else { //Proceso de insertar a segundo plano
            for (i = 0; i < MAXOPEN; i++) {
                if(ForkStr[i].pid<0) break;
            }
            ForkStr[i].pid = pid;

            ForkStr[i].senal = 0;
            strcpy(ForkStr[i].nombresenal,"ACTIVO");
            ...
            strcpy(ForkStr[i].linea, lin);
            ForkStr[i].senal = 0;
            strcpy(ForkStr[i].usuario, getlogin());
            ForkStr[i].prioridad = prior;
            ...
            inft = localtime(&t);
            char fecha[30];
            strftime(fecha, sizeof(fecha), "%Y/%m/%d %H:%M:%S", inft);
            strcpy(ForkStr[i].hora, fecha);
        }
    }
}
...
int intruccion(int maxtr, char * tr[maxtr], tList list, struct CommandsStruct CoSt, struct MemoryStruct MemStr[], struct SharedStruct ShaStr[], struct MmapStruct MapStr[], struct ProcessList PList[], struct ProcForkList ForkList[]) {
    ...
    if(strcmp(tr[0], "exit")==0 ||strcmp(tr[0], "bye")==0||strcmp(tr[0], "quit")==0) { //Salir del programa
        ...
        authors(tr[1]);
    } else if(strcmp(tr[0], "comand")==0) { //Repite un comando
        //comand(tr[1], list);
        return 2;
    } else if(strcmp(tr[0], "hist")==0) { //Historial
    ...
    else if(strcmp(tr[0],"malloc")==0){ //--------P2
        MAlloc(tr, MemStr);
    ...
    else { //Comprueba si se quiso introducir un comando de terminal
        lastcheck(tr, ForkList);
    }
...
int main() {
    ...
    struct ProcForkList ForkList[MAXOPEN];
    for (int i = 0; i < MAXOPEN; ++i) {
        ForkList[i] = (struct ProcForkList) {-1, "", "", 0,"", 0};
    }
    ...
    while (exit==false) {
        ...
        if (strcmp(input, "\n") != 0) { //Evita meter nulls al programa o cadenas vacias
            ...
            checker = intruccion(maxtr, tr, list, CoSt, MemStr, ShaStr, MapStr, PList, ForkList);
            ...
            switch (checker) {
                ...
                case 2: //Si retorna 2 se uso comand, que repite una instruccion. Esta implementacion esta pensada para evitar el bucle infinito en el que se llama a si mismo
                    if(tr[1] != NULL) {
                        ...
                        intruccion(maxtr, tr, list, CoSt, MemStr, ShaStr, MapStr, PList, ForkList);
                    }
    ...
    //Antes de acabar, vacia la lista de procesos (y los libera), todas las asignaciones de memoria y borra la lista del historial
    CloseAllOpen(PList);
